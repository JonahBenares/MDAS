...
if(isset($_POST['save_summary'])){
    ...
    for($y=1;$y<$count_outage;$y++){
        ...
        $existing = mysqli_query($conn, "SELECT summary_id FROM outage_summary_luzon WHERE summary_id = '$sid'");
        $count = mysqli_num_rows($existing);

        //echo $count . "<br>";
        if($count == 0){
            if(!empty($outage_type)){
        $insert = mysqli_query($conn, "INSERT INTO outage_summary_luzon (summary_id, outage_type, remarks) VALUES ('$sid','$outage_type','$remarks');");
            }
        } else {
    ...
    <div class="container-fluid" style="background-color: #fff">
        ...
        <table class="table table-hover table-bordered" width="100%">
            <tr>
                ...
                <td colspan="5"><center><h5 class="m-t-20"><b>Actual Outages (Luzon)</b></h5></center></td>
                <td colspan="2">
                    <!-- <center>
        ...
        <form method='POST' >
            <div class="m-b-70">
                <table class="table table-hover table-bordered" width="100%">
                    ...
                    <?php
                        ...
                        $outage_type = get_column($conn, "outage_type", "outage_summary_luzon", "summary_id", $fetch_outage['summary_id']);
                        ...
                        <tr>
                            ...
                            <td class="p-0">
                                <select class="form-control" name='outage_type<?php echo $a; ?>'>
                            ...
                            <td class="p-0">
                                <textarea class="form-control" name="remarks<?php echo $a; ?>" rows="1"><?php echo $remarks; ?></textarea>
                            </td>
                            <td width="2%" class="">
            ...
            <div style="height: 100px;width: 100%;background-image: linear-gradient(to top, #fff,#fff0);position: fixed;bottom:0">
                <div style="padding:60px 20px;width: 97%">
                    <input type="submit" class="btn btn-success-alt btn-sm btn-block" value="Save" name="save_summary">
                </div>
            </div>
        </form>
    </div>
