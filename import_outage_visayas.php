...
while($fetch_mpsv = mysqli_fetch_array($mps_v)){
	//$min_hour = get_min_hour($conn, $fetch_mpsv['delivery_date'], $fetch_mpsv['resource_id']);
	 ...
	 $outage_count = get_count($conn, 'mps_visayas', $fetch_mpsv['delivery_date'], $fetch_mpsv['resource_id']);
	...
	if($outage_count==24){
		...
		$min_hour = get_min_hour($conn, 'mps_visayas', $fetch_mpsv['delivery_date'], $fetch_mpsv['resource_id']);
